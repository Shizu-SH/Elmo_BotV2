...
let minar = `${pickRandom(['✿ Dejaste que un extraño te toque las nalgas a cambio de dinero, ganaste',
'✿ Ayudaste al admin a eyacular y te dió',
'✿ Participaste en un juego atrevido al compartir tu sensualidad sin inhibiciones. Ganaste',
'✿ Se la mamastes al creador del Bot y ganaste'])}`
...
let time = user.lastmiming + 150000 //10 min
if (new Date - user.lastmiming < 150000) return await conn.reply(m.chat, `*⏱️ 𝙑𝙪𝙚𝙡𝙫𝙖 𝙚𝙣 ${msToTime(time - new Date())} 𝙥𝙖𝙧𝙖 𝙘𝙤𝙣𝙩𝙞𝙣𝙪𝙖𝙧 𝙢𝙞𝙣𝙖𝙣𝙙𝙤 ${global.imagen5('exp')}⛏️*\n\n*𝙂𝙚𝙩 𝙗𝙖𝙘𝙠 𝙞𝙣 ${msToTime(time - new Date())} 𝙩𝙤 𝙢𝙞𝙣𝙚 ${global.imagen5('exp')}⛏️*`, fkontak,  m)
